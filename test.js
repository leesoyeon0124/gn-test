// test.js (SECURE VERSION: Randomization + Anti-Cheat)
...
// Configuration
let randomizedQuestions = []; // Will hold the shuffled questions
let currentSectionIndex = 0;
let userAnswers = {};
let startTime = 0; // Timestamp for total time calculation

// 6 Sections x 10 Questions 
// Note: Sections are now page-based (0 to 5), not ID-based, because questions are shuffled.
const SECTION_COUNT = 6;
const QUESTIONS_PER_SECTION = 10;
const SECTION_COLORS = [
    "--section-1-color", // Page 1
    "--section-1-color", // Page 2
    "--section-2-color", // Page 3
    "--section-2-color", // Page 4
    "--section-3-color", // Page 5
    "--section-3-color"  // Page 6
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check start time
    if (!localStorage.getItem('gnFit_startTime')) {
        const now = Date.now();
        localStorage.setItem('gnFit_startTime', now.toString());
        startTime = now;
    } else {
        startTime = parseInt(localStorage.getItem('gnFit_startTime'), 10);
    }
...
function initializeTest() {
    // 1. Check if we already have a randomized order saved
    const savedOrder = localStorage.getItem('gnFit_questionOrder');
    if (savedOrder) {
        // Restore order
        const indices = JSON.parse(savedOrder);
        randomizedQuestions = indices.map(id => allQuestionsSource.find(q => q.id === id));
    } else {
        // New Shuffle
        randomizedQuestions = [...allQuestionsSource];
        shuffleArray(randomizedQuestions);

        // Save order (only IDs to save space)
        const indices = randomizedQuestions.map(q => q.id);
        localStorage.setItem('gnFit_questionOrder', JSON.stringify(indices));
    }
...
function renderCurrentSection() {
    ...
    const startIdx = currentSectionIndex * QUESTIONS_PER_SECTION;
    const endIdx = startIdx + QUESTIONS_PER_SECTION;
    const currentQuestions = randomizedQuestions.slice(startIdx, endIdx);

    // 1. Update UI Theme
    const colorVar = SECTION_COLORS[currentSectionIndex] || "--section-1-color";
    document.documentElement.style.setProperty('--theme-color', `var(${colorVar})`);

    // Update body class for background transitions if needed
    // (Simplifying to use just one class since theme color handles most)
    bodyEl.className = `section-${Math.floor(currentSectionIndex / 2) + 1}`;

    // 2. Render HTML
    questionListEl.innerHTML = currentQuestions.map(q => createQuestionHTML(q)).join('');

    // 3. Restore Answers & Update Progress
    updateProgressUI();
}
...
function updateProgressUI() {
    const totalQuestions = allQuestionsSource.length; // 60
    ...
    const pageAnsweredCount = currentQuestions.filter(q => userAnswers[q.id]).length;
    const isPageComplete = pageAnsweredCount === currentQuestions.length;
    ...
    if (isPageComplete) {
        ...
        if (currentSectionIndex === SECTION_COUNT - 1) {
...
submitBtn.addEventListener('click', async () => {
    ...
    const record = {
        ...
        phone,
        agree: "Y",
        answers, // This allows GAS to map "1", "2"... keys correctly
        totalTime: durationSeconds, // Added totalTime
        timestamp
    };

    // 3. Save to Local Admin DB
    ...
    if (GOOGLE_SCRIPT_URL && GOOGLE_SCRIPT_URL.startsWith("https://script.google.com")) {
        ...
        try {
            ...
            // Even if it fails, we assume local save is safe.
            alert("전송 중 문제가 발생했으나, 로컬에 저장되었습니다.");
        }
    } else {
